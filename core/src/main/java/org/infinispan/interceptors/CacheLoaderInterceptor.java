...
public class CacheLoaderInterceptor extends JmxStatsCommandInterceptor {
   ...
   public Object visitKeySetCommand(InvocationContext ctx, KeySetCommand command) throws Throwable {
      ...
      if (useClusterSize == null || !useClusterSize.equalsIgnoreCase("true")) {
         if (enabled && !hasSkipLoadFlag(command)) {
            ...
            return Immutables.immutableSetWrap(union);
         }
      }
      return keys;
   }
   ...
   public Object visitValuesCommand(InvocationContext ctx, ValuesCommand command) throws Throwable {
      ...
      if (useClusterSize == null || !useClusterSize.equalsIgnoreCase("true")) {
         if (enabled && !hasSkipLoadFlag(command)) {
            ...
            return Immutables.immutableCollectionWrap(result);
         }
      }
      return values;
   }
   ...
   private boolean skipLoad(FlagAffectedCommand cmd, Object key, InvocationContext ctx) {
      ...
      if (cmd instanceof GetKeyValueCommand) {
         ...
         if (trace) {
            log.tracef("Skip load for read command %s? %s", cmd, skip);
         }
         ...
         if (trace) {
            log.tracef("Unknown command type %s. Don't skip load.", cmd);
         }
      }
      return skip;
   }
