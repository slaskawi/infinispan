...
public class DistributedEntryRetriever<K, V> extends LocalEntryRetriever<K, V> {
   ...
   class IterationStatus<C> {
      ...
      final AtomicReference<Address> awaitingResponseFrom = new AtomicReference<Address>();
   ...
   private <H, C extends H> void startRetrievingValues(final UUID identifier, final Set<Integer> segments,
      ...
      final Set<Integer> inDoubtSegments = new HashSet<Integer>(segments.size());
      ...
      if (canTryProcess) {
            ...
            public void run() {
               ...
               while (repeat) {
                  ...
                  try {
                     ...
                     // TODO: need to replace the map action
                     ParallelIterableMap.KeyValueAction<? super K, InternalCacheEntry> action =
                     ...
                     Collection entriesToSend = new ArrayList<CacheEntry>(queue);
         ...
         executorService.execute(new Runnable() {
            ...
            public void run() {
               ...
               Collection<CacheEntry> emptyEntries = Collections.emptySet();
   ...
   public <C> CloseableIterator<CacheEntry> retrieveEntries(KeyValueFilter<? super K, ? super V> filter,
      ...
      DistributedItr<C> itr = new DistributedItr<C>(batchSize, identifier, listener, hash);
      ...
      final IterationStatus<C> status = new IterationStatus<C>(itr, listener, filter, usedConverter, flags, processedKeys);
   ...
   private Set<Integer> findMissingLocalSegments(AtomicReferenceArray<Set<K>> processValues, ConsistentHash hash) {
      Set<Integer> ourSegments = hash.getPrimarySegmentsForOwner(localAddress);
      Set<Integer> returnSegments = new HashSet<Integer>();
   ...
   private Map.Entry<Address, Set<Integer>> findOptimalRoute(Set<Integer> segmentsToFind, ConsistentHash hash) {
      ...
      routes = new HashMap<Address, Set<Integer>>();
   ...
   private <C> void processData(final UUID identifier, Address origin, Set<Integer> completedSegments, Set<Integer> inDoubtSegments,
      ...
      if (status != null) {
         ...
         Collection<CacheEntry> nonDuplicateEntries = new ArrayList<CacheEntry>(entries.size());
         Map<Integer, ConcurrentHashSet<K>> finishedKeysForSegment = new HashMap<Integer, ConcurrentHashSet<K>>();
   ...
   protected class DistributedItr<C> extends Itr<C> {
      ...
      private final ConcurrentMap<Integer, Set<K>> keysNeededToComplete = new ConcurrentHashMap<Integer, Set<K>>();
