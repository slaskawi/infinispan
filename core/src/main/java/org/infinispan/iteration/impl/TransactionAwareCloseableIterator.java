...
public class TransactionAwareCloseableIterator<K, C> extends RemovableEntryIterator<K, C> {
   ...
   public TransactionAwareCloseableIterator(CloseableIterator<CacheEntry> realIterator,
                                            TxInvocationContext ctx, Cache<K, ?> cache) {
      super(realIterator, cache, false);
      this.ctx = ctx;
      contextEntries = new ArrayList<CacheEntry>(ctx.getLookedUpEntries().values());
