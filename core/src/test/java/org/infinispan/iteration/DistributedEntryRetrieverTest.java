...
public class DistributedEntryRetrieverTest extends BaseClusteredEntryRetrieverTest {
   ...
   public void verifyNodeLeavesBeforeGettingData() throws TimeoutException, InterruptedException, ExecutionException {
      ...
      final BlockingQueue<CacheEntry> returnQueue = new ArrayBlockingQueue<CacheEntry>(10);
   ...
   public void verifyNodeLeavesAfterSendingBackSomeData() throws TimeoutException, InterruptedException, ExecutionException {
      ...
      final BlockingQueue<CacheEntry> returnQueue = new LinkedBlockingQueue<CacheEntry>();
      Future<Void> future = fork(new Callable<Void>() {
         @Override
         public Void call() throws Exception {
            Iterator<CacheEntry> iter = retriever.retrieveEntries(null, null, null, null);
      ...
      CacheEntry value = returnQueue.poll(10, TimeUnit.SECONDS);
   ...
   public void waitUntilProcessingResults() throws TimeoutException, InterruptedException, ExecutionException {
      ...
      Map<Integer, Set<CacheEntry>> expected = generateEntriesPerSegment(hash, values.entrySet());
   ...
   private Map<Integer, Set<CacheEntry>> generateEntriesPerSegment(ConsistentHash hash, Iterable<? extends Map.Entry> entries) {
      Map<Integer, Set<CacheEntry>> returnMap = new HashMap<Integer, Set<CacheEntry>>();

      for (Map.Entry value : entries) {
         int segment = hash.getSegment(value.getKey());
         Set<CacheEntry> set = returnMap.get(segment);
         if (set == null) {
            set = new HashSet<CacheEntry>();
