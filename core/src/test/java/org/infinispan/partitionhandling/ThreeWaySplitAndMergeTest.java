...
public class ThreeWaySplitAndMergeTest extends BasePartitionHandlingTest {
   ...
   private void testSplitAndMerge(PartitionDescriptor p0, PartitionDescriptor p1, PartitionDescriptor p2) throws Exception {
      ...
      Set<Address> members = new HashSet<Address>(Arrays.asList(new Address[]{address(p0.node(0)), address(p0.node(1)), address(p1.node(0))}));
      assertEquals(new HashSet<Address>(advancedCache(p0.node(0)).getDistributionManager().getConsistentHash().getMembers()), members);
      assertEquals(new HashSet<Address>(advancedCache(p0.node(1)).getDistributionManager().getConsistentHash().getMembers()), members);
      assertEquals(new HashSet<Address>(advancedCache(p1.node(0)).getDistributionManager().getConsistentHash().getMembers()), members);

      partition(1).assertKeysNotAvailableForRead(k0, k1, k2, k3);

      members = new HashSet<Address>(Arrays.asList(new Address[]{address(0), address(1), address(2), address(3)}));
      assertEquals(new HashSet<Address>(advancedCache(p2.node(0)).getDistributionManager().getConsistentHash().getMembers()), members);
