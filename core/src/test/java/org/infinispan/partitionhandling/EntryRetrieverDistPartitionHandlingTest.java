...
public class EntryRetrieverDistPartitionHandlingTest extends BasePartitionHandlingTest {
   ...
   public void testRetrievalWhenPartitionIsDegrated() {
      ...
      EntryIterable iterable = cache(0).getAdvancedCache().filterEntries(AcceptAllKeyValueFilter.getInstance());
      try {
         iterable.iterator();
      } finally {
         iterable.close();
   ...
   public void testRetrievalWhenPartitionIsDegratedButLocal() {
      ...
      EntryIterable iterable = cache(0).getAdvancedCache().withFlags(Flag.CACHE_MODE_LOCAL).filterEntries(
   ...
   public void testUsingIterableButPartitionOccursBeforeGettingIterator() throws InterruptedException {
      ...
      EntryIterable<MagicKey, String> iterable = cache0.getAdvancedCache().filterEntries(AcceptAllKeyValueFilter.getInstance());
      try {
         ...
         try {
            Iterator<CacheEntry> iterator = iterable.iterator();
   ...
   public void testUsingIteratorButPartitionOccursBeforeRetrievingRemoteValues() throws InterruptedException {
      ...
      CloseableIterator<CacheEntry> iterator = cache0.getAdvancedCache().filterEntries(
