...
transaction associated to this call. It’s length is determined by the
...
.Infinispan versions
TIP: This version of the protocol is implemented since Infinispan 7.0.0.Alpha2.
...
One new flag has been added too:
...
can be failed-over to other nodes.
* +0x88+ = Illegal lifecycle state. When a client receives this error as response,
it means that the server server-side cache or cache manager are not available
for requests because either stopped, they're stopping or similar situation.
Generally, requests that return this error can be failed-over to other nodes.
