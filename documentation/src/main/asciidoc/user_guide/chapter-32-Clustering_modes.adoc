...
- split brains. Caused e.g. by a router crash, this splits the cluster in two or more
...
ways to alleviate the possibility for the split brain to happen, such as redundant networks or
 ...
 These only reduces the window for the problem to occur, though.
...
but sacrifice availability in the presence of partitions
...
bank account information). On the other hand, if Infinispan is used as an best-effort
cache, e.g. to cache web pages in a proxy, the partitions are perfectly tolerable.

The following sections describes the way Infinispan handles split brains and
...
this decision based on in future, based on user feedback.
