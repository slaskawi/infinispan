...
- split brains. Caused e.g. by a router crash, this splits the cluster in two or more partitions, or sub-clusters that that operate independently. In these circumstances multiple clients reading/writing from different partitions see different versions of the same cache entry, which for many application is problematic. Note there are ways to alleviate the possibility for the split brain to happen, such as redundant networks or
 link:$$http://www.cyberciti.biz/tips/linux-bond-or-team-multiple-network-interfaces-nic-into-single-interface.html$$[IP bonding]. These only reduces the window for the problem to occur, though.
...
The partition handling functionality discussed in this section allows the user to be informed when data has been lost,
temporarily or permanently, in order to avoid situations in which wrong data is shown to the user as a result of either split brains or multiple node crash in rapid sequence.
More formally, enabling partition handling in Infinispan preserves data consistency but sacrifice availability in the presence of partitions (CP in Brewer's link:$$http://en.wikipedia.org/wiki/CAP_theorem$$[`CAP theorem]).

The following sections describes the way Infinispan handles split brains and successive failures when partition handling is enabled, followed by a section on configuring the partition handling functionality.

==== Split brain

A partition is considered to happen when *numOwners* or more nodes disappear at the same time. When this happen two (or more) partitions form which are not aware of each other. Each such partition does not start a rebalance,
but enters in degraded mode:

- request (read and writes) for entries that have all the copies on nodes within this partition are honoured
...
To exemplify, consider the initial cluster C={A,B,C,D}, A,B,C,D - nodes, configured in distributed mode with numOwners=2. Further on,
the cluster contains k1, k2 and k3 keys such that owners(k1) = {A,B}, owners(k2) = {B,C} and owners(k3) = {C,D}. Then a partition happens C1={A,B} and C2={C,D}, the degraded mode exhibits the following behaviour:
...
A relevant aspect of the partition handling process is the fact that when a split brain happens, the resulting partitions rely on the original consistent hash function (the one that existed before the split brain) in order
 to calculate key ownership. So it doesn't matter if k1, k2 or k3 already exists in the cluster or not, as the availability is strictly determined by the consistent hash and not by the key existence.

If at a further point in time the initial partition C forms again as a result of the network healing and C1 and C2 partitions being merged back together, then C exists the degraded mode becoming fully available again.

===== Partition handling for replicated caches
Infinispan internally handles a replicated cache as a degenerated distributed mode where *numOwners* equals the cluster size. Following the logic described above, all the partitions of a replicated cache that suffers a split brain would be available,
as for every such partition the number of nodes that left is smaller than *numOwners* (which in turn is equal with the initial cluster size). Having both partitions available defeats the main purpose of the partition handling,
allowing inconsistencies to happen: e.g. an update on the first partition is allowed and at the same time readers getting the old information from the other partition.
This partition handling strategy doesn't work with replicated caches, where other strategies such as majority of nodes
are more appropriate. Because of this, the partition handling functionality is not available for replicated caches at the moment and it is ignored.

==== Successive node failures
When an Infinispan node leaves, either because of being shut down or because of a (e.g. process) crash, the remaining owners of the data
pertaining to the crashed node send copies of this data to other nodes in the cluster in order to preserve the *numOwners* invariant.
This process is called rebalance. However, another node might crash during rebalance and in the situation more than *numOwners* nodes
disappear, there is a change that data disappeared from the cluster altogether. In this case with partition handling functionality enabled,
the cluster enters in unavailable mode and all the requests are answered with an AvailabilityException. At this stage there is no way for the
cluster to become available again, but stopping it and repopulating it on restart with the data from an external source.
Even though the  successive node failures situation is not expected to happen often as an appropriate *numOwners* should be configured in order to avoid it,
 having the partition handling enabled represent a safety net for systems that have critical consistency requirements.

==== Configuration for partition handling functionality

At this stage the partition handling is disabled by default. We will revisit this decision based on in future, based on user feedback.
...
Note: there are plans to add JMX monitoring operations in order to forcefully migrate a cluster form degraded or unavailable state back to available, at the cost of consistency.
