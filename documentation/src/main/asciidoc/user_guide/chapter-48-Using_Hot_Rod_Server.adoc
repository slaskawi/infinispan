...
library for this purpose. The `SerializationContext.registerProtofile` method receives the name of a classpath resource
that is expected to be a serialized protobuf binary descriptor containing our type declarations. The binary descriptor
(.protobin) is compiled with the help of Protobuf's _protoc_ generator tool using the _--descriptor_set_out_ command
line option out of the 'library.proto' file we created earlier. It's basically just a more compact equivalent
representation of our file. Future Infinispan versions will be able to accept the .proto file directly
(see link:$$https://issues.jboss.org/browse/ISPN-3480$$[ISPN-3480]). Generating binary descriptors from .proto files is
described link:$$https://developers.google.com/protocol-buffers/docs/techniques#self-description$$[here].
...
possible if the entry is protobuf encoded. The server needs to extract the relevant metadata from the same binary descriptor
(.protobin file) as the client. The descriptor is currently supplied to the server by remotely invoking the
