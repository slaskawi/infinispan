...
Java Hot Rod client users can still develop `Converter` or `KeyValueFilter`
...
instance. If doing this and deploying `Converter` or `KeyValueFilter` instances,
the same marshaller instance needs to be deployed in the server so that callback parameters
of `Converter` or `KeyValueFilter` instances can be correctly unmarshalled.

To deploy a Marshaller instance server-side, follow a similar method to the one
used to deploy `Converter` or `KeyValueFilter` instances:
...
same jar as the `Converter` and/or `KeyValueFilter` instances. Also, currently
