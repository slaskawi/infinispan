...
The client API maps the local API: link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/client/hotrod/RemoteCacheManager.html$$[RemoteCacheManager] corresponds to link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/manager/DefaultCacheManager.html$$[DefaultCacheManager] (both implement link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/manager/CacheContainer.html$$[CacheContainer] ). This common API facilitates an easy migration from local calls to remote calls through Hot Rod: all one needs to do is switch between link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/manager/DefaultCacheManager.html$$[DefaultCacheManager] and link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/client/hotrod/RemoteCacheManager.html$$[RemoteCacheManager] - which is further simplified by the common link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/manager/CacheContainer.html$$[CacheContainer] interface that both inherit.
...
===== Versioned API
A RemoteCacheManager provides instances of link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/client/hotrod/RemoteCache.html$$[RemoteCache] interface that represents a handle to the named or default cache on the remote cluster. API wise, it extends the link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/Cache.html$$[Cache] interface to which it also adds some new methods, including the so called versioned API. Please find below some examples of this API <<sid-68355052, but to understand the motivation behind it, make sure you read this section>>.
...
For more details on versioned operations refer to link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/client/hotrod/RemoteCache.html$$[RemoteCache] 's javadoc.
...
particular client listener. To enable filtering, a key/value filter factory
...
class StaticKeyValueFilterFactory implements KeyValueFilterFactory {
   @Override
...
class StaticKeyValueFilter implements KeyValueFilter<Integer, String>, Serializable {
   @Override
...
The key/value filter factory instance defined above creates filter instances
which statically filter out all entries except the one whose key is `1`.

To be able to register a listener with this key/value filter factory,
the factory has to be given a unique name, and the Hot Rod server needs to be
plugged with the name and the key/value filter factory instance. Plugging the
...
2. Create a `META-INF/services/org.infinispan.filter.KeyValueFilterFactory` file
...
On top of that, the client listener needs to be linked with this key/value
...
class DynamicKeyValueFilterFactory implements KeyValueFilterFactory {
   @Override
   public KeyValueFilter<Integer, String> getKeyValueFilter(Object[] params) {
...
// Serializable, Externalizable or marshallable with Infinispan Externalizers
// needed when running in a cluster
class DynamicKeyValueFilter implements KeyValueFilter<Integer, String>, Serializable {
   final Object[] params;

   DynamicKeyValueFilter(Object[] params) {
      this.params = params;
   }
...
NOTE: Hot Rod servers store data as byte arrays, but in spite of that,
Java Hot Rod client users can still develop `KeyValueFilter` instances that
worked on typed objects. The way this is done is by making the Hot Rod server
use the same marshaller as the one used by the Java Hot Rod client. This is
enabled by default.
...
contain even less information. This customization is done with `Converter`
instances generated by a `ConverterFactory`:
...
class StaticConverterFactory implements ConverterFactory {
   final Converter<Integer, String, CustomEvent> staticConverter = new StaticConverter();
...
// Serializable, Externalizable or marshallable with Infinispan Externalizers
// needed when running in a cluster
class StaticConverter implements Converter<Integer, String, CustomEvent>, Serializable {
...
2. Create a `META-INF/services/org.infinispan.filter.ConverterFactory` file
within the JAR file and within it, write the fully qualified class name of the
...
class DynamicConverterFactory implements ConverterFactory {
   public Converter<Integer, String, CustomEvent> getConverter(final Object[] params) {
...
class DynamicConverter implements <Integer, String, CustomEvent>, Serializable {
   final Object[] params;

   DynamicConverter(Object[] params) {
      this.params = params;
   }
...
NOTE: Hot Rod servers store data as byte arrays, but in spite of that,
Java Hot Rod client users can still develop `Converter` instances that
worked on typed objects. The way this is done is by making the Hot Rod server
use the same marshaller as the one used by the Java Hot Rod client. This is
enabled by default.
...
link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/client/hotrod/RemoteCache.html$$[RemoteCache] offers alternative versioned methods for these atomic operations, that are also network friendly, by not sending the whole value object over the network, but a version identifier. See the section on versioned API.

Each one of these unsupported operation is documented in the link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/client/hotrod/RemoteCache.html$$[RemoteCache] javadoc.
...
implementations of link:$$http://docs.jboss.org/infinispan/7.0/apidocs/org/infinispan/client/hotrod/impl/transport/tcp/RequestBalancingStrategy.html$$[RequestBalancingStrategy]
...
The number of connections per server, total number of connections, how long should a connection be kept idle in the pool before being closed - all these (and more) can be configured. Please refer to the javadoc of link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/client/hotrod/RemoteCacheManager.html$$[RemoteCacheManager] for a list of all possible configuration elements.
...
The marshaller can be plugged through the "marshaller" configuration element (see Configuration section): the value should be the fully qualified name of a class implementing the link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/marshall/Marshaller.html$$[Marshaller] interface. This is a optional parameter, if not specified it defaults to the link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/marshall/jboss/GenericJBossMarshaller.html$$[GenericJBossMarshaller] - a highly optimized implementation based on the link:$$http://www.jboss.org/jbossmarshalling$$[JBoss Marshalling] library.
...
===== Statistics
Various server usage statistics can be obtained through the link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/client/hotrod/RemoteCache.html$$[RemoteCache] .stats() method. This returns a link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/client/hotrod/ServerStatistics.html$$[ServerStatistics] object - please refer to javadoc for details on the available statistics.
...
*  link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/client/hotrod/RemoteCacheManager.html$$[RemoteCacheManager]
*  link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/client/hotrod/RemoteCache.html$$[RemoteCache]
...
The most common case is for a client to use a Hot Rod client library directly to store data in the Hot Rod server. In this case, assuming that the client used the existing Java Hot Rod client, the default marshaller used to marshall objects into byte arrays is the link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/marshall/jboss/GenericJBossMarshaller.html$$[GenericJBossMarshaller] . So, if a user wants to read data from the Hot Rod server directly, it would need to execute something along the lines of:
...
dedicated marshaller, link:$$http://docs.jboss.org/infinispan/6.0/apidocs/org/infinispan/client/hotrod/marshall/ProtoStreamMarshaller.html$$[_ProtoStreamMarshaller_].
...
link:$$https://github.com/infinispan/infinispan/blob/6.0.0.CR1/client/hotrod-client/src/test/java/org/infinispan/client/hotrod/query/HotRodQueryTest.java$$[HotRodQueryTest]
